/* R:\Repos\OS\src\common\Font.psf (3/23/2019 9:25:26 AM)
   StartOffset(h): 00000000, EndOffset(h): 0000081F, Length(h): 00000820 */

#include "FontPC.hpp"
   
unsigned char rawData[2080] = 
{
...
namespace FontPC
{
	...
	bool UseForground(char letter, int x, int y)
	{
		psf* font = (psf*)rawData;
		unsigned char *glyph = (unsigned char*)rawData + font->headersize 
			+ (letter < font->numglyph ? letter : 0) * font->bytesperglyph;
			
		int bytesperline = (font->width + 7) / 8;
		int mask = 1 << (font->width - 1);
		
		glyph += bytesperline * y;
		mask >>= x;
		
		return ((int)*glyph) & mask ? true : false;
	}
}
