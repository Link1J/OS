...
extern "C" EFI_STATUS efi_main(EFI_HANDLE imgHandle, EFI_SYSTEM_TABLE* sysTable)
{
	...
	Console::Print(L"Finding ACPI 2.0 Table\r\n");
	
	int a = 0;
	for (; a < sysTable->NumberOfTableEntries; a++)
		if (sysTable->ConfigurationTable[a].VendorGuid.Data1 == guid.Data1)
			if (sysTable->ConfigurationTable[a].VendorGuid.Data2 == guid.Data2)
				if (sysTable->ConfigurationTable[a].VendorGuid.Data3 == guid.Data3)
					if (sysTable->ConfigurationTable[a].VendorGuid.Data4[0] == guid.Data4[0])
						if (sysTable->ConfigurationTable[a].VendorGuid.Data4[1] == guid.Data4[1])
							if (sysTable->ConfigurationTable[a].VendorGuid.Data4[2] == guid.Data4[2])
								if (sysTable->ConfigurationTable[a].VendorGuid.Data4[3] == guid.Data4[3])
									if (sysTable->ConfigurationTable[a].VendorGuid.Data4[4] == guid.Data4[4])
										if (sysTable->ConfigurationTable[a].VendorGuid.Data4[5] == guid.Data4[5])
											if (sysTable->ConfigurationTable[a].VendorGuid.Data4[6] == guid.Data4[6])
												if (sysTable->ConfigurationTable[a].VendorGuid.Data4[7] == guid.Data4[7])
													break;
	
	if (a == sysTable->NumberOfTableEntries)
	{
		Console::Print(L"Failed to find ACPI 2.0 Table\r\nPress any key to exit...\r\n");
        EFIUtil::WaitForKey();
        return EFI_LOAD_ERROR;
    }
    ...
    FileIO::FileData kernelData = FileIO::ReadFile(L"kernel.elf");
	...
	header->RSDPStructure = Paging::ConvertPtr(sysTable->ConfigurationTable[a].VendorTable);
