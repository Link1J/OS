...
void initPaging(KernelHeader* header)
{
	...
	uint64_t* pagingBuffer = (uint64_t*)allocate(4096 + 4096 + 512 * 4096, (EFI_MEMORY_TYPE)0x80000001);

	for(int i = 0; i < 512; i++)
		pagingBuffer[i] = 0;
	// Make 512 GB at 0 and 512 GB at 0xFFFFFF8000000000 map to the same PML3
	pagingBuffer[0] = pagingBuffer[511] = PML_SET_ADDR((uint64_t)&pagingBuffer[512]) | PML_SET_P(1) | PML_SET_RW(1);

	// Fill all PML3 entries with the addresses of the corresponding PML2s
	for(int i = 0; i < 512; i++)
		pagingBuffer[512 + i] = PML_SET_ADDR((uint64_t)&pagingBuffer[1024 + 512 * i]) | PML_SET_P(1) | PML_SET_RW(1);
	...
	g_HighMemBase = ((uint64_t)511 << 39) | 0xFFFF000000000000;   // This is the starting address of physical ram in the higher half virtual memory
        
	header->pageBuffer = (uint64_t*)convertPtr(pagingBuffer);
...
void* convertPtr(void* pointer)
{
	...
	return (void*)((uint64_t)pointer + g_HighMemBase);
}
